...
def run_conversion():
    ...
    try:
        ...
        cursor.execute(
            "SELECT protocolo, conteudo_json, origem_lista FROM solicitacoes_raw"
        )

        logger.info(
            "⚙️ Iniciando processamento pelo Mapper DDD (Clean Break) em chunks (SRE)..."
        )
        ...
        while True:
            ...
            for c_date in colunas_data:
                if c_date in df_chunk.columns:
                    df_chunk[c_date] = pd.to_datetime(
                        df_chunk[c_date], format="%d/%m/%Y %H:%M:%S", errors="coerce"
                    )
            ...
            if "SLA_Interacoes_Regulacao" in df_chunk.columns:
                df_chunk["SLA_Interacoes_Regulacao"] = (
                    pd.to_numeric(df_chunk["SLA_Interacoes_Regulacao"], errors="coerce")
                    .fillna(0)
                    .astype(int)
                )

            if "SLA_Desfecho_Atingido" in df_chunk.columns:
                df_chunk["SLA_Desfecho_Atingido"] = df_chunk[
                    "SLA_Desfecho_Atingido"
                ].astype(bool)
            ...
            for col in df_chunk.columns:
                if (
                    col not in colunas_data
                    and col not in cols_cat
                    and col not in cols_bool
                    and col not in float_cols
                    and col != "SLA_Interacoes_Regulacao"
                ):
                    # Força cast para string e limpa representações de nulo
                    df_chunk[col] = (
                        df_chunk[col].astype(str).replace(["None", "nan", "<NA>"], "")
                    )
            ...
            logger.info(
                f"⚡ Chunk processado. Total acumulado: {processed_total} registros. Erros: {errors}"
            )
        ...
        logger.info(
            f"🎉 Pipeline SRE concluído! Arquivo gerado: {parquet_out} ({processed_total} registros)"
        )
