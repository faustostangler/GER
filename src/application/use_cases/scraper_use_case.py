...
class ScraperUseCase(IScrapingUseCase):
    ...
    def _evaluate_circuit_breaker(self):
        ...
        if ratio > self.CB_THRESHOLD_RATIO:
            logger.critical(
                f"🚀 CIRCUIT BREAKER DISPARADO! Taxa de falha/schema é de {ratio:.1%} > {self.CB_THRESHOLD_RATIO:.1%}. Abortando para salvar S3 e Custo de DLQ!"
            )
            raise DomainContractViolationException(
                f"API do Vendor Quebrou Radicalmente com índice de {ratio * 100}% Poison Pills!"
            )
