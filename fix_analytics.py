...
for i, line in enumerate(lines):
    if line.startswith("# --- 3. STATE MANAGEMENT ---"):
        skip = True
        new_lines.append(
            "from src.presentation.components.filters import clear_filter_state, render_include_exclude, render_boolean_radio, render_presence_radio, render_dual_slider, render_age_slider, render_smart_date_range, render_advanced_text_search\n"
        )
        new_lines.append(
    ...
    if not skip:
        # replace usages
        line = re.sub(
            r"render_include_exclude\(", r"render_include_exclude(use_case, ", line
        )
