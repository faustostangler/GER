...
def get_watermark_from_db(chave: str):
    ...
    try:
        ...
        cur  = conn.cursor()
        cur.execute("SELECT MAX(data_alteracao) FROM solicitacoes_raw WHERE origem_lista = ?", (chave,))
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for c in candidates_ms:
            probe_filter(page, f"params.{c} = {ts_ms}",
                         f"params.{c} = {ts_ms};", baseline)

        logger.info("")
        # ── 3. Candidatos em ISO string ───────────────────────────────────
        logger.info("── ISO STRING (YYYY-MM-DDTHH:MM:SS) ─────────────────")
        for c in candidates_ms:
            probe_filter(page, f"params.{c} = '{ts_iso_full}'",
                         f"params.{c} = '{ts_iso_full}';", baseline)

        logger.info("")
        # ── 4. Candidatos em data simples YYYY-MM-DD ──────────────────────
        logger.info("── DATE ONLY (YYYY-MM-DD) ────────────────────────────")
        for c in candidates_ms:
            probe_filter(page, f"params.{c} = '{ts_iso_date}'",
                         f"params.{c} = '{ts_iso_date}';", baseline)

        logger.info("")
        # ── 5. Candidatos com Z suffix ────────────────────────────────────
        logger.info("── ISO COM Z (UTC) ───────────────────────────────────")
        for c in candidates_ms:
            probe_filter(page, f"params.{c} = '{ts_iso_z}'",
                         f"params.{c} = '{ts_iso_z}';", baseline)
