...
def test_request(page, description, custom_params_js):
    ...
    for index, item in enumerate(data):
        ...
        (item.get("classificacaoRisco") or {}).get("pontosGravidade", 0)
        pt_temp = (item.get("classificacaoRisco") or {}).get("pontosTempo", 0)
        (item.get("classificacaoRisco") or {}).get("totalPontos", 0)

        logger.info(
            f"  #{index + 1} Prot: {prot} | Alterado: {dt_alt} | Solic: {dt_solic} | Pts Tempo: {pt_temp}"
        )
...
def main():
    ...
    with sync_playwright() as p:
        ...
        test_request(
            page,
            "Sort Data (params.sort='dataAlterouUltimaSituacao')",
            "params.sort = 'dataAlterouUltimaSituacao';",
        )
        test_request(
            page,
            "Sort Data Desc (params.sort='dataAlterouUltimaSituacao,desc')",
            "params.sort = 'dataAlterouUltimaSituacao,desc';",
        )

        test_request(
            page,
            "Ord. Data (params.ordenacao='dataAlterouUltimaSituacao')",
            "params.ordenacao = 'dataAlterouUltimaSituacao';",
        )
        test_request(
            page,
            "Ord. Data Desc (params.ordenacao='dataAlterouUltimaSituacao,desc')",
            "params.ordenacao = 'dataAlterouUltimaSituacao,desc';",
        )

        test_request(
            page,
            "OrderBy Data (params.orderBy='dataAlterouUltimaSituacao')",
            "params.orderBy = 'dataAlterouUltimaSituacao';",
        )
        test_request(
            page,
            "OrderBy Data Desc (params.order='desc')",
            "params.orderBy = 'dataAlterouUltimaSituacao'; params.order= 'desc';",
        )

        test_request(
            page,
            "Sort Pontos Tempo",
            "params.sort = 'classificacaoRisco.pontosTempo,desc';",
        )
        test_request(
            page,
            "Ord. Pontos Tempo",
            "params.ordenacao = 'classificacaoRisco.pontosTempo,desc';",
        )

        test_request(
            page,
            "Filtro 7 Dias (params.dataInicio...)",
            """
        ...
        test_request(
            page,
            "Filtro 7 Dias Alteracao (params.dataInicioAlteracaoStatus)",
            """
            let d = new Date();
            d.setDate(d.getDate() - 7);
