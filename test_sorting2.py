...
def test_request(page, description, custom_params_js):
    ...
    for index, item in enumerate(data):
        ...
        logger.info(f"  #{index+1} Prot: {prot} | Alterado: {dt_alt} | Solic: {dt_solic}")

def main():
    if not USER or not PASS: return
    with sync_playwright() as p:
        ...
        test_request(page, "Ord: -dataAlterouUltimaSituacao", "params.ordenacao = ['-dataAlterouUltimaSituacao'];")
        test_request(page, "Ord: +dataAlterouUltimaSituacao", "params.ordenacao = ['+dataAlterouUltimaSituacao'];")
        
        test_request(page, "Ord: -dataSolicitacao", "params.ordenacao = ['-dataSolicitacao'];")
        
        test_request(page, "Filtro 7 Dias (dataInicioSolicitacao)", """
            let d = new Date(); d.setDate(d.getDate() - 7);
            params.dataInicioSolicitacao = d.toISOString();
        """)
        
        test_request(page, "Filtro 7 Dias (dataInicioAlteracaoUltimaSituacao)", """
            let d = new Date(); d.setDate(d.getDate() - 7);
            params.dataInicioAlteracaoUltimaSituacao = d.toISOString();
        """)

        test_request(page, "Filtro 7 Dias (dataInicial)", """
            let d = new Date(); d.setDate(d.getDate() - 7);
