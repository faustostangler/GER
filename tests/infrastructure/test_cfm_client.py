...
async def test_cfm_client_validate_success():
    ...
    with patch("httpx.AsyncClient.get") as mock_get:
        mock_get.return_value = AsyncMock(
            status_code=200, json=lambda: {"status": "success", "active": True}
        )
...
async def test_cfm_client_validate_inactive():
    ...
    with patch("httpx.AsyncClient.get") as mock_get:
        mock_get.return_value = AsyncMock(
            status_code=200, json=lambda: {"status": "success", "active": False}
        )
...
async def test_cfm_client_validate_http_error():
    ...
    with patch("httpx.AsyncClient.get") as mock_get:
        ...
        client = CFMClient()
        with pytest.raises(
            ConnectionError, match="Falha na comunicação com a API do CFM"
        ):
