...
async def test_consumer_forwards_to_dlq_after_max_retries():
    ...
    with (
        patch(
            "infrastructure.events.keycloak_kafka_consumer.AIOKafkaConsumer",
            new=MockConsumer,
        ),
        patch(
            "infrastructure.events.keycloak_kafka_consumer.AIOKafkaProducer"
        ) as MockProducer,
        patch(
            "infrastructure.events.keycloak_kafka_consumer.validate_cfm_api"
        ) as mock_validate,
        patch(
            "infrastructure.events.keycloak_kafka_consumer.is_already_processed"
        ) as mock_idempotency,
        patch(
            "infrastructure.events.keycloak_kafka_consumer.asyncio.sleep",
            return_value=None,
        ),
    ):
...
async def test_consumer_skips_non_register_events():
    ...
    """
    event_payload = {"id": str(uuid.uuid4()), "type": "LOGIN", "userId": "some_user"}
    ...
    with (
        ...
        patch(
            "infrastructure.events.keycloak_kafka_consumer._process_register_event"
        ) as mock_process,
    ):
        mock_idempotency.return_value = False
        mock_producer_instance = AsyncMock()
