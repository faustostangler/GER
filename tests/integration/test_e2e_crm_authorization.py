...
def setup_teardown_db():
    ...
    with engine.connect() as conn:
        result = conn.execute(
            text(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='doctor_profiles'"
            )
        )
...
async def test_e2e_crm_authorization_flow():
    ...
    mock_event = {
        ...
        "type": "REGISTER",
        "userId": user_id,
        "details": {"crm_numero": crm_numero, "crm_uf": crm_uf},
    }
    ...
    with (
        patch(
            "infrastructure.auth.jwt_validator.jwks_client.get_signing_key_from_jwt"
        ) as mock_jwks,
