...
def test_cdc_streamlit_requires_specific_kpis_fields():
    ...
    assert "pacientes" in fields, (
        "CDC Quebrado: Streamlit necessita de AnalyticKPIs.pacientes para renderizar o card de volume total."
    )
    assert "lead_time" in fields, (
        "CDC Quebrado: Streamlit necessita de AnalyticKPIs.lead_time para compor o SLA."
    )
    assert "p90_lead_time" in fields, (
        "CDC Quebrado: Streamlit necessita de AnalyticKPIs.p90_lead_time para a métrica estatística de outliers."
    )
    ...
    try:
        ...
        pytest.fail(
            f"O contrato estrutural inicial foi violado ao tentar instanciar o objeto com as chaves obrigatórias: {e}"
        )
