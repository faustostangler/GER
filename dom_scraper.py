...
def extract_data_from_json(j):
    ...
    data["Data do Primeiro Agendamento"] = timestamp_to_date(
        j.get("dataPrimeiroAgendamento")
    )
    ...
    if u:
        ...
        data["Data de Nascimento"] = (
            ...
            if timestamp_to_date(u.get("dataNascimento"))
            else ""
        )
                    ...
                    if evo_item.get("codigo") == "regionalizacao":
                        ...
                        for evo_item_linha in evo_item_texto_reg.split("\n"):
                            ...
                            if ":" in evo_item_linha:
                                evo_item_chave, evo_item_valor = evo_item_linha.split(
                                    ":", 1
                                )
                                data[f"Regionalização {evo_item_chave.strip()}"] = (
                                    evo_item_valor.strip()
                                )
    ...
    data["Unidade Solicitante Descrição"] = (usol.get("tipoUnidade") or {}).get(
        "descricao", ""
    )
...
def main():
    ...
    with sync_playwright() as p:
        ...
        logger.info(
            "Login aparente realizado. Aguardando carregamento da interface inicial..."
        )
        ...
        try:
            ...
            logger.debug(
                "Botão de unidade não encontrado ou não necessário. Prosseguindo..."
            )
            pass
        ...
        try:
            ...
            logger.info(
                "Tabela carregada! Iniciando raspagem invisível da Fila de Espera..."
            )
        except Exception:
        ...
        if existing_protocols:
            logger.info(
                f"Encontrados {len(existing_protocols)} protocolos já salvos no CSV. Registros existentes serão atualizados."
            )
        ...
        while True:
            ...
            if total_pages is not None and page_num > total_pages:
                logger.info(
                    f"--- Concluído: Todas as {total_pages} páginas projetadas foram extraídas! ---"
                )
                break
            ...
            if total_pages is not None:
                ...
                avg_time_per_page = (
                    elapsed_time / pages_completed if pages_completed > 0 else 0
                )
                cadastros_por_seg = (
                    cadastros_completed / elapsed_time if elapsed_time > 0 else 0
                )
                ...
                percent = (
                    (pages_completed / total_pages * 100) if total_pages > 0 else 0
                )
                ...
                payload_str = (
                    f"{format_size(batch_bytes)}/{format_size(total_bytes)}"
                    if total_bytes
                    else ""
                )
            ...
            try:
                ...
                try:
                    logger.debug(
                        "Navegando de volta para a URL base do Gercon para verificar estado da sessão..."
                    )
                    ...
                    logger.debug(
                        f"Página recarregada e estabilizada. URL atual é: {main_page.url}"
                    )

                    # Verifica se caímos na tela de login do SSO (sso-pmpa.procempa.com.br)
                    if main_page.locator("#username").count() > 0:
                        logger.warning(
                            "Queda para a tela do SSO detectada! Refazendo login de forma invisível..."
                        )
                        main_page.fill("#username", USER)
                        main_page.fill("#password", PASS)
                        main_page.click("#kc-login")
                        main_page.wait_for_load_state("networkidle")
                        logger.debug(
                            f"Login re-enviado. Nova URL pós-login: {main_page.url}"
                        )
                        ...
                        try:
                            logger.debug(
                                "Aguardando possível tela de seleção de unidade do Gercon..."
                            )
                            main_page.wait_for_selector(
                                f"xpath={xpath_btn}", timeout=TIMEOUT
                            )
                            main_page.locator(f"xpath={xpath_btn}").click()
                            main_page.wait_for_load_state("networkidle")
                            logger.debug(
                                "Unidade re-selecionada no fluxo de recuperação."
                            )
                        except Exception:
                            logger.debug(
                                "Seleção de unidade não exigida no fluxo de recuperação."
                            )
                    ...
                    logger.info(
                        "Sistema reconectado com sucesso! Retomando coleta da página..."
                    )
                except Exception as ex:
                    logger.error(
                        f"Falha ao recuperar sessão logada. Encerrando. ({ex})"
                    )
                    break
                continue
            ...
            logger.debug(
                f"[main_loop] JS resolvido! Lote com {len(jsons)} registros capturado. Fila total aferida: {total_dados}. Download neste lote: {(bytes_neste_lote / 1024):.2f}KB. Total Acumulado: {(payload_bytes_total / 1024 / 1024):.2f}MB"
            )
            ...
            if (
                time.time() - last_ping_time > TIMEOUT / 1000 * 60
            ):  # 30 * 60 # 30 minutos
                try:
                    ...
                    logger.debug(
                        f"[Ping] Resultado da renovação de Token: {ping_result}"
                    )
