...
def render_advanced_text_search(label: str, column: str, clauses: list, key: str):
    ...
    with st.popover(f"🔎 {label}", use_container_width=True):
        ...
        st.caption("Separe as expressões por vírgula ( , )")
...
def main():
    ...
    with st.sidebar.expander("🌍 Demografia & Rede", expanded=False):
        ...
        v_nmin = num_min.number_input("Número Min", value=0, step=10, key="nmin_adv")
        v_nmax = num_max.number_input("Número Max", value=99999, step=100, key="nmax_adv")
        if v_nmin > 0 or v_nmax < 99999: clauses.append(f"TRY_CAST(\"Número\" AS INTEGER) BETWEEN {v_nmin} AND {v_nmax}")
        
        dt_nasc = st.date_input("Data Nascimento (Range)", value=[], key="dt_nasc_adv")
    ...
    with st.sidebar.expander("🩺 Clínico & Regulação", expanded=False):
        ...
        st.markdown("---")
        render_advanced_text_search("CID Código", "CID Código", clauses, "txt_cid_cod")
    ...
    with st.sidebar.expander("⚠️ Triagem & Pontuação", expanded=False):
        ...
        curr_where = render_range_slider("Pontos Gravidade", "Pontos Gravidade", clauses, "pt_grav")
        curr_where = render_range_slider("Pontos Tempo", "Pontos Tempo", clauses, "pt_tmp")
        curr_where = render_range_slider("Pontuação Total", "Pontuação", clauses, "pt_tot")

    # --- 5. GOVERNANÇA E ATORES ---
    with st.sidebar.expander("🏛️ Governança & Atores", expanded=False):
        oj = st.radio("Ordem Judicial", ["Ambos", "Sim", "Não"], horizontal=True, key="oj_adv")
        ...
        st.markdown("---")
        render_advanced_text_search("Médico Solicitante", "Médico Solicitante", clauses, "txt_med")
        render_advanced_text_search("Operador do Sistema", "Operador", clauses, "txt_op")
        render_advanced_text_search("Usuário Solicitante", "Usuário Solicitante", clauses, "txt_usr")
    ...
    with t_geral:
        ...
        with c1:
            ...
            st.plotly_chart(fig1, key="plt_sit_adv", use_container_width=True)
        with c2:
            ...
            st.plotly_chart(fig2, key="plt_list_adv", use_container_width=True)
    ...
    with t_loc:
        ...
        with c1:
            ...
            if not df_mun.empty:
                try:
                    ...
                    st.plotly_chart(fig_tree, key="plt_tree_adv", use_container_width=True)
                except Exception:
        ...
        with c2:
            ...
            st.plotly_chart(fig_bar, key="plt_demo_adv", use_container_width=True)
    ...
    with t_clin:
        ...
        if not df_esp.empty:
            try:
                ...
                st.plotly_chart(fig_sun, key="plt_sun_adv", use_container_width=True)
            except Exception:
        ...
        with c1:
            ...
            st.plotly_chart(fig_cid, key="plt_cid_adv", use_container_width=True)
        with c2:
            ...
            st.plotly_chart(fig_risc, key="plt_risc_adv", use_container_width=True)
    ...
    with t_perf:
        ...
        with c1:
            ...
            st.plotly_chart(fig_in, key="plt_tin_adv", use_container_width=True)
        with c2:
            ...
            st.plotly_chart(fig_ev, key="plt_tev_adv", use_container_width=True)
        ...
        st.plotly_chart(fig_tipo, key="plt_life_adv", use_container_width=True)

    # 5. TAB: DATA GRID (MICRO)
    with t_micro:
        st.subheader("Auditoria Clínica")
        limit = st.slider("Amostra (Linhas)", 10, 500, 50, key="sld_limit_adv")
        df_grid = query_db(f"""
